...
$to = "tetsuson2017shuttensha@gmail.com";
...
<title>CONTACT | TETSUSON2017</title>
<style type="text/css">
/* 自由に編集下さい */
footer{
  position: relative;
...
<article>
	...
	<div id="formWrap">
		...
		<?php }else{ ?>
		<p align="center">以下の内容でよろしければ、ページ下部の送信ボタンを押してください。<br>誤りがある場合は、前画面に戻って再度ご入力ください。</p>
		<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
		<table class="formTable">
		<?php echo confirmOutput($_POST);//入力内容を表示?>
		</table>
		<p align="center" id="button" class="clearfix">
			<input type="hidden" name="mail_set" value="confirm_submit">
			<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
			<input type="button" value="戻る" alt="戻る" onClick="history.back()"/>
			<input type="submit" value="送信" alt="送信" />
		</p>
		</form>
		<?php } ?>
	</div><!-- /formWrap -->
